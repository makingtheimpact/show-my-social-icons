...
define('SMSI_UPDATE_CACHE_TIME', 12 * HOUR_IN_SECONDS); // Check every 12 hours

function smsi_check_for_updates($transient) {
    if (empty($transient->checked)) {
        return $transient;
    }

    // Check if we already have cached update data
    $cached_response = get_transient('smsi_update_check');
    if ($cached_response !== false) {
        if ($cached_response === 'no_update') {
            return $transient;
        }
        if (is_object($cached_response)) {
            $transient->response['show-my-social-icons/show-my-social-icons.php'] = $cached_response;
            return $transient;
        }
    }
    ...
    $plugin_file = $plugin_dir . '/show-my-social-icons.php';
    ...
    $proxy_url = add_query_arg(array(
        'plugin_slug' => 'show-my-social-icons',
    ...
    try {
        ...
        if (!empty($update_data['new_version']) && !empty($update_data['package'])) {
            if (version_compare($update_data['new_version'], $current_version, '>')) {
                ...
                set_transient('smsi_update_check', $update_object, SMSI_UPDATE_CACHE_TIME);
...
add_filter('site_transient_update_plugins', 'smsi_check_for_updates');

// Function to handle the update process
function smsi_update_plugin($transient) {
...
function smsi_clear_update_cache() {
    delete_transient('smsi_update_check');
}

add_action('upgrader_process_complete', 'smsi_clear_update_cache', 10, 0);
add_action('deleted_plugin', 'smsi_clear_update_cache');
add_action('activated_plugin', 'smsi_clear_update_cache');
add_action('deactivated_plugin', 'smsi_clear_update_cache');
