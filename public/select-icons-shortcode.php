...
function smsi_select_icons_shortcode($atts) {
    $atts = shortcode_atts(
        array(
            'platforms' => [],
        ...
        $atts,
        'select_my_social_icons'
    );

    $platforms = is_array($atts['platforms']) ? $atts['platforms'] : explode(',', $atts['platforms']);
    $platforms = array_map('trim', $platforms); // Trim whitespace
    $platforms = array_filter($platforms); // Remove empty values

    if (empty($platforms)) {
        return __('No platforms selected.', 'show-my-social-icons');
    }
    ...
    foreach ($platforms as $platform) {
        ...
        if ($url) {
            ...
            if (!empty($icon_html)) {
                $ordered_icons[] = "<div class='smsi-icon-wrapper' style='margin-right: " . esc_attr($single_icon_margin_right) . "; margin-left: " . esc_attr($single_icon_margin_left) . ";'>{$icon_html}</div>";
            }
        }
    }

    $icon_output = implode('', $ordered_icons);
    $html = "<div class='smsi-icons-wrapper' style='" . esc_attr($container_style) . "'>" . $icon_output . "</div>";
    return $html;
}

add_shortcode('select_my_social_icons', 'smsi_select_icons_shortcode');
